...
```{r}
stanmodel <- stan_model('DA_23.stan')
```
...
inits <- function() {
  list(r =   2.416431e-01 ,
       ...
       sd_obs1 = 2.486435e+05, 
       sd_obs2 =2.911327e-01)
}
```
...
```{r}
summary(DA_23_fit) #FIT SUMMARY
```

DIAGNOSTIC PLOTS FOR STAN FIT

```{r}
rstan::traceplot(DA_23_fit, pars = "r", inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = "Q", inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = "a1", inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = "e1", inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = "d1", inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = "h1", inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = "w1", inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = "B", inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = 'sd_obs1', inc_warmup = TRUE)

rstan::traceplot(DA_23_fit, pars = 'sd_obs2', inc_warmup = TRUE)
```
...
Q_prior <- data.frame(x = seq(1e-8,1e-3, by = 1e-6), vals =
  dtnorm(x= seq(1e-8,1e-3, by = 1e-6), mean = 1e-7, sd = 1e-7, lower = 1e-8, upper = 1e-3))

a1_prior <- data.frame(x = seq(0,2, by = 0.001), vals =
  dtnorm(x= seq(0,2, by = 0.001), mean =0.2, sd = 1, lower = 0, upper = 2))

e1_prior <- data.frame(x = seq(0,0.01, by = 1e-6), vals =
  dtnorm(x= seq(0,0.01, by = 1e-6), mean = 1e-3, sd = 1, lower = 0, upper = 0.01))

d1_prior <- data.frame(x = seq(0,3.5, by = 0.001), vals =
  dtnorm(x= seq(0,3.5, by = 0.001), mean = 0.2, sd = 1, lower = 0, upper = 3.5))
...
```{r}
stan_dens(DA_23_fit, pars = c("r"), separate_chains = TRUE) +
   geom_line(data = r_prior, aes(x=x, y = vals), linewidth = 0.75)

stan_dens(DA_23_fit, pars = c("Q"), separate_chains = TRUE) +
    geom_line(data = Q_prior, aes(x=x, y = vals), linewidth = 0.75)

stan_dens(DA_23_fit, pars = c("a1"), separate_chains = TRUE) +
    geom_line(data = a1_prior, aes(x=x, y = vals), linewidth = 0.75)

stan_dens(DA_23_fit, pars = c("e1"), separate_chains = TRUE) +
  geom_line(data = e1_prior, aes(x=x, y = vals), linewidth = 0.75)

stan_dens(DA_23_fit, pars = c("d1"), separate_chains = TRUE)+
    geom_line(data = d1_prior, aes(x=x, y = vals), linewidth = 0.75)

stan_dens(DA_23_fit, pars = c("h1"), separate_chains = TRUE)+
    geom_line(data = h1_prior, aes(x=x, y = vals), linewidth = 0.75)

stan_dens(DA_23_fit, pars = c("w1"), separate_chains = TRUE) +
    geom_line(data = w1_prior, aes(x=x, y = vals), linewidth = 0.75)

stan_dens(DA_23_fit, pars = c("B"), separate_chains = TRUE) +
    geom_line(data = B_prior, aes(x=x, y = vals), linewidth = 0.75)

stan_dens(DA_23_fit, pars = c("sd_obs1"), separate_chains = TRUE)

stan_dens(DA_23_fit, pars = c("sd_obs2"), separate_chains = TRUE)
```

```{r}
pairs(DA_23_fit, pars = c("r", "Q", "a1", "e1", "d1", "h1", "w1", "B"), log = FALSE)
```

SAVE STAN FIT AND DRAWS

```{r}
save(DA_23_fit, file = "DA_23_fit") # SAVE FIT
```
...
DA_23_draws_iter <- extract(DA_23_fit, pars = c('r', 'Q', 'a1', 'e1', 'd1', 'h1', 'w1', 'B', 'N1_0', 'N2_0'), inc_warmup = FALSE, permuted = TRUE)

DA_23_draws_iter <- as.data.frame(DA_23_draws_iter)

DA_23_draws_iter <- tibble::rownames_to_column(DA_23_draws_iter, "Iteration")

write_csv(DA_23_draws_iter, file = "DA_23_draws_iter.csv")
```
...
DA_23_pars_sum <- extract(DA_23_fit, pars = c('r', 'Q', 'a1', 'e1', 'd1', 'h1', 'w1', 'B', 'N1_0', 'N2_0'), inc_warmup = FALSE, permuted = FALSE)

DA_23_pars_sum <- monitor(DA_23_pars_sum, warmup = 0, print = FALSE, digits_summary = 7)

DA_23_pars_sum <- as.data.frame(DA_23_pars_sum)

DA_23_pars_sum <- tibble::rownames_to_column(DA_23_pars_sum, "Parameter")

write_csv(DA_23_pars_sum, file = "DA_23_pars_sum.csv")
