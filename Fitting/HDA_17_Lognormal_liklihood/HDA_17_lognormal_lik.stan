///////////////////////////////////////////////////////////////////////////////////////
////////////////////////// STAN CODE FOR THREE SPECIES JARS 17C /////////////////////////
///////////////////////////////////////////////////////////////////////////////////////
...
model {
  ...
  r ~ normal(9.091239e-02, 7.869849e-03);   //Priors from DA_17
  
  Q ~ normal(7.921066e-08, 4.197810e-08);   //Priors from DA_17
  
  a1 ~ normal(5,3);
  
  h1 ~ normal(4.376354e-06, 2.214301e-06);   //Priors from DA_17
  
  w1 ~ normal(1.956673e+00, 2.877751e-01);   //Priors from DA_17
  
  
  //////////Pred priors from HDA_20/////////

  a2 ~ normal(1.860873e+01, 4.227891e+00);
  
  //a2 ~ (0,30);

  e2 ~ normal(1.151926e-01, 1.612833e-02);
  
  //e2 ~ uniform(0,1);

  d2 ~ normal(3.318828e-02, 7.723955e-03);
  
  //d2 ~ uniform(0,0.5);

  h2 ~ normal(2.231816e-01, 5.352247e-02);
  
  //h2 ~ uniform(0,1);

  w2 ~ normal(3.103368e+01, 8.051515e+00);
...
for (i in 1:m) {
  N1_0[i] ~ normal(N1[i,1],1.5e4);
  N2_0[i] ~ normal(N2[i,1],0.2);
  N3_0[i] ~ normal(N3[i,1],0.1);
}
  
  sd_obs1 ~ exponential(1e-5);
  sd_obs2 ~ exponential(1);
  log_sd_obs3 ~ exponential(1);

  
    ////////////////////////// DEFINE PARS FOR INTEGRATOR ///////////////////////// 
  
  ///////////////////////// fixed vals from median of DA_17_fit////////////////////
 ...
 // Initial conditions (still normal for simplicity)
N1[j,1] ~ normal(N1_0[j], sd_obs1);
N2[j,1] ~ normal(N2_0[j], sd_obs2);
N3[j,1] ~ lognormal(log(N3_0[j] + 1e-6), log_sd_obs3);  // CHANGED from normal to lognormal

for (i in 2:n) {
  // Algae (N1): stays normal
  N1[j,i] ~ normal(Nsim[i-1, 1], sd_obs1);

  // Ceriodaphnia (N2): stays normal due to zeros
  N2[j,i] ~ normal(Nsim[i-1, 2], sd_obs2);

  // Hydra (N3): now lognormal
  N3[j,i] ~ lognormal(log(Nsim[i-1, 3] + 1e-6), log_sd_obs3); // CHANGED // 1e-6 added to avoid log(0)
}
